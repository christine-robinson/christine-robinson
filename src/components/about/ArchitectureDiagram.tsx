  ...
  return (
    <div className="panel relative overflow-hidden p-5">
      ...
      <div className="relative">
        <p className="font-mono type-label text-muted-foreground">PLATFORM LAYERS</p>
        <div className="mt-5 space-y-3">
          {LAYERS.map((layer, i) => (
            <div key={layer.title} className="relative">
              <div className="rounded-md border border-border bg-surface-2/60 p-3.5">
                ...
                <div className="mt-2.5 flex flex-wrap gap-1.5">
                  {layer.items.map((item) => (
                    <span
                      key={item}
                      className="rounded border border-border/70 bg-background/50 px-2 py-0.5 font-mono type-badge text-primary/90"
                    >
                      {item}
                    </span>
                  ))}
              ...
              {i < LAYERS.length - 1 ? (
                <svg className="mx-auto my-1 block h-3 w-4" viewBox="0 0 16 12" aria-hidden="true">
                  <line
                    x1="8"
                    y1="0"
                    x2="8"
